...
public class DefaultAlipayServiceImpl implements AlipayServiceAPI {
    ...
    public DefaultAlipayServiceImpl(FtpUtil ftpUtil) {
    ...
    public AlipayInfoVO getQRCode(String orderId) {
        ...
        if (filepath == null || filepath.trim().length() == 0) {
            ...
            AlipayInfoVO infoVO = new AlipayInfoVO();
            infoVO.setOrderId(orderId);
            infoVO.setQRCodeAddress(filepath);
            return infoVO;
        }
    }
    ...
    public AlipayResultVO getOrderStatus(String orderId) {
        // 获取支付状态
        boolean query = tradeQuery(orderId);
        if (query) {
    ...
    /**
     * 当面付2.0查询订单
     * @param orderId (必填) 商户订单号，通过此商户订单号查询当面付的交易状态
     * @return
     */
...
//        goodsDetailList.add(goods2);
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                filePath = String.format("/Users/huangyaning/视频资料/Dubbo项目/资料/qrcode/qr-%s.png",
